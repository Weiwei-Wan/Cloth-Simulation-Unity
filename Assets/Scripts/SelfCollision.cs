...
public class SelfCollision : MonoBehaviour {
    ...
    int fixedNode1 = node_row_num*9+8;
    int fixedNode2 = node_row_num*9+8;
    ...
    Vector3[] normals = new Vector3[node_num];
    ...
    void CreateSphere(int index, Vector3 pos) {
        GameObject sp = GameObject.CreatePrimitive(PrimitiveType.Sphere);
        sp.transform.position = pos;
        sp.transform.localScale = Vector3.zero;
        //sp.transform.localScale = new Vector3(springIniLen, springIniLen, springIniLen);

        NodeSpheres[index] = sp;
    }
    ...
    void ComputeJacobianForce()
    {
        ...
        force[fixedNode1]=Vector3.zero;
        force[fixedNode2]=Vector3.zero;
        ...
        while (it < it_max) {
            ...
            rho_old = rho;
            pos[fixedNode1]=pos_pre[fixedNode1];
            pos[fixedNode2]=pos_pre[fixedNode2];
        }
    }
    ...
    void Assemble()
    {
        ...
        for (int i=0; i<node_num; i++) {
            // pin node
            if (i==fixedNode1 || i==fixedNode2) {continue;}
            ...
            Vector3 spherePos = pos[i];
            float safe_pos = 0.02f;
            // collision with previous nodes
            for (int j=0; j<i-1; j++) {
                int x_i = i/node_row_num;
                int y_i = i%node_row_num;

                int x = j/node_row_num;
                int y = j%node_row_num;

                if (Mathf.Abs(x-x_i) < 4 || Mathf.Abs(y-y_i) < 4) {continue;}

                if (hasThisPos(x,y-1) && hasThisPos(x-1, y-1)) {
                    if (rayTriangle.IsRayTriangleCollision(pos_pre[i], spherePos, pos[j], pos[x*node_row_num+y-1], pos[(x-1)*node_row_num+y-1])) {
                        spherePos = rayTriangle.GetHitPos(pos_pre[i], spherePos, pos[j], pos[x*node_row_num+y-1], pos[(x-1)*node_row_num+y-1]);
                    }
                }
                if (hasThisPos(x-1,y) && hasThisPos(x-1, y-1)) {
                    if (rayTriangle.IsRayTriangleCollision(pos_pre[i], spherePos, pos[j], pos[(x-1)*node_row_num+y], pos[(x-1)*node_row_num+y-1])) {
                        spherePos = rayTriangle.GetHitPos(pos_pre[i], spherePos, pos[j], pos[(x-1)*node_row_num+y], pos[(x-1)*node_row_num+y-1]);
                    }
                }
            }
            pos[i] = spherePos;
            // NodeSpheres[i].transform.position = pos[i];
